...
public abstract class LimbAnimatorMixin {
    ...
    public float getPos(float original) {
        if (dontvisuals.getInstance().getModuleManager().getModule(NoRender.class).isToggled() && dontvisuals.getInstance().getModuleManager().getModule(NoRender.class).limbs.getValue()) return 0;
