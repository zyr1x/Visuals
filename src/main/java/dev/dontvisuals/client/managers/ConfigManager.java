...
import dev.dontvisuals.modules.settings.impl.StringSetting;
import dev.dontvisuals.modules.settings.impl.EnumSetting;
import dev.dontvisuals.modules.settings.impl.ColorSetting;
...
public class ConfigManager implements Wrapper {
    ...
    public ConfigManager() {
        this.configsDir = new File(dontvisuals.getInstance().getGlobalsDir(), "configs");
    ...
    public CompletableFuture<Boolean> saveConfig(String configName) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                try {
                    String pref = dontvisuals.getInstance().getCommandManager().getPrefix();
                ...
                for (Module module : dontvisuals.getInstance().getModuleManager().getModules()) {
                    ...
                    for (Setting<?> setting : module.getSettings()) {
                        ...
                        if (setting instanceof ColorSetting) {
                            ...
                            dev.dontvisuals.modules.settings.api.Bind bind = (dev.dontvisuals.modules.settings.api.Bind) value;
                            String modeName = bind.getMode() != null ? bind.getMode().name() : dev.dontvisuals.modules.settings.api.Bind.Mode.TOGGLE.name();
                ...
                for (HudElement hudElement : dontvisuals.getInstance().getHudManager().getHudElements()) {
                    ...
                    try {
                        ...
                        dev.dontvisuals.modules.settings.impl.ListSetting elementsList = dontvisuals.getInstance().getHudManager().getElements();
                        dev.dontvisuals.modules.settings.impl.BooleanSetting bs = elementsList.getName(hudElement.getName());
                ...
                try {
                    ...
                    try {
                        ...
                        if (pref != null && !pref.isEmpty()) {
                            dontvisuals.getInstance().getCommandManager().setPrefix(pref);
                        }
                    } catch (Exception ignored) {}
                    ...
                    for (Map.Entry<String, ModuleData> entry : configData.getModules().entrySet()) {
                        ...
                        Module module = dontvisuals.getInstance().getModuleManager().getModuleByName(moduleName);
                        if (module != null) {
                    ...
                    try {
                        ...
                        for (ThemeManager.Theme theme : availableThemes) {
                            if (theme.getName().equals(configData.getCurrentTheme())) {
                                ...
                                dontvisuals.getInstance().getEventHandler().post(new dev.dontvisuals.client.events.impl.EventThemeChanged(theme));
                                break;
                            }
                        }
                    } catch (Exception e) {
                    ...
                    if (configData.getHudPositions() != null) {
                        try {
                            for (Map.Entry<String, HudPositionData> entry : configData.getHudPositions().entrySet()) {
                                ...
                                HudElement hudElement = dontvisuals.getInstance().getHudManager().getHudElements().stream()
                                ...
                                if (hudElement != null) {
                                    ...
                                    try {
                                        dev.dontvisuals.modules.settings.impl.ListSetting elementsList = dontvisuals.getInstance().getHudManager().getElements();
                                        dev.dontvisuals.modules.settings.impl.BooleanSetting bs = elementsList.getName(hudName);
        ...
        if (setting instanceof BooleanSetting) {
            ...
            if (value instanceof String) {
                try {
                    ...
                    dev.dontvisuals.modules.settings.api.Bind.Mode mode = dev.dontvisuals.modules.settings.api.Bind.Mode.TOGGLE;
                    if (parts.length > 2) {
                        try {
                            mode = dev.dontvisuals.modules.settings.api.Bind.Mode.valueOf(parts[2]);
                        } catch (IllegalArgumentException ignored) {}
                    }
                    ((BindSetting) setting).setValue(new dev.dontvisuals.modules.settings.api.Bind(key, isMouse, mode));
                } catch (Exception e) {
    ...
    public static class ModuleData {
        ...
        private dev.dontvisuals.modules.settings.api.Bind bind;
        ...
        public dev.dontvisuals.modules.settings.api.Bind getBind() {
            return bind;
        }
        
        public void setBind(dev.dontvisuals.modules.settings.api.Bind bind) {
            this.bind = bind;
        }
