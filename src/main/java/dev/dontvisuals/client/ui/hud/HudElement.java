package dev.dontvisuals.client.ui.hud;
...
import dev.dontvisuals.modules.settings.api.Position;
import dev.dontvisuals.modules.settings.impl.BooleanSetting;
import dev.dontvisuals.modules.settings.impl.PositionSetting;
...
import static dev.dontvisuals.modules.api.Module.fullNullCheck;
...
public abstract class HudElement extends Module {
    ...
    public void onRender2D(EventRender2D e) {
        ...
        if (button) {
            ...
            dev.dontvisuals.client.ui.hud.HudElement current = dontvisuals.getInstance().getHudManager().getCurrentDragging();
            ...
            if (!(mc.currentScreen instanceof ChatScreen)) {
                dragging = false;
                button = false;
                dontvisuals.getInstance().getHudManager().setCurrentDragging(null);
                return;
            }
            ...
            if (!dragging && MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY())) {
                ...
                dragY = mouseY() - getY();
                dragging = true;
                dontvisuals.getInstance().getHudManager().setCurrentDragging(this);
            }
...
//            e.getContext().drawTextWithShadow(mc.textRenderer, text, x, y, 0xFFFFFF);
    ...
    public void onRender2DX2(EventRender2D e) {
        ...
        Setting<?> setting = dontvisuals.getInstance().getHudManager().getElements().getName(getName());
    ...
    public void onMouse(EventMouse e) {
        ...
        if (e.getAction() == 0) {
            ...
            if (dontvisuals.getInstance().getHudManager().getCurrentDragging() == null ||
                dontvisuals.getInstance().getHudManager().getCurrentDragging() == this) {
                button = true;
            }
            ...
            if (e.getButton() == 1) {
                if (MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY())) {
                    ...
                    if (settings.size() == 1) {
                        ...
                        if (dontvisuals.getInstance().getHudManager().getWindow() != null)
                            dontvisuals.getInstance().getHudManager().getWindow().reset();
                        for (HudElement element : dontvisuals.getInstance().getHudManager().getHudElements()) {
